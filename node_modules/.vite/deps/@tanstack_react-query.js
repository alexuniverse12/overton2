  ...
  "node_modules/use-sync-external-store/cjs/use-sync-external-store-shim.development.js"(exports) {
    ...
    if (true) {
      (function() {
        ...
        var useState5 = React10.useState, useEffect5 = React10.useEffect, useLayoutEffect = React10.useLayoutEffect, useDebugValue = React10.useDebugValue;
...
function useQueries({
  ...
  const defaultedQueries = React3.useMemo(() => queries.map((options) => {
    const defaultedOptions = queryClient.defaultQueryOptions(options);
    defaultedOptions._optimisticResults = isRestoring ? "isRestoring" : "optimistic";
    return defaultedOptions;
  }), [queries, queryClient, isRestoring]);
  const [observer] = React3.useState(() => new QueriesObserver(queryClient, defaultedQueries));
  const result = observer.getOptimisticResult(defaultedQueries);
  useSyncExternalStore(React3.useCallback((onStoreChange) => isRestoring ? () => void 0 : observer.subscribe(notifyManager.batchCalls(onStoreChange)), [observer, isRestoring]), () => observer.getCurrentResult(), () => observer.getCurrentResult());
  React3.useEffect(() => {
...
var QueryErrorResetBoundary = ({
  ...
  return React4.createElement(QueryErrorResetBoundaryContext.Provider, {
    value
...
function useIsFetching(arg1, arg2, arg3) {
  ...
  return useSyncExternalStore(React7.useCallback((onStoreChange) => queryCache.subscribe(notifyManager.batchCalls(onStoreChange)), [queryCache]), () => queryClient.isFetching(filters), () => queryClient.isFetching(filters));
}
...
function useIsMutating(arg1, arg2, arg3) {
  ...
  return useSyncExternalStore(React8.useCallback((onStoreChange) => mutationCache.subscribe(notifyManager.batchCalls(onStoreChange)), [mutationCache]), () => queryClient.isMutating(filters), () => queryClient.isMutating(filters));
}
